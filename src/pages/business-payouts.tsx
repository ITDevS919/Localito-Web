...
export default function BusinessPayoutsPage() {
  ...
  const fetchEarnings = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/business/earnings`, {
        credentials: "include",
      });
  ...
  const fetchPayouts = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/business/payouts`, {
        credentials: "include",
      });
      const data = await res.json();
      if (res.ok && data.success) {
  ...
  const fetchSettings = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/business/payout-settings`, {
        credentials: "include",
      });
      const data = await res.json();
      if (res.ok && data.success) {
  ...
  const fetchStripeStatus = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/business/stripe/status`, {
        credentials: "include",
      });
      const data = await res.json();
      if (res.ok && data.success) {
  ...
  const handleRequestVerification = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/business/payout-settings/request-verification`, {
        method: "POST",
      ...
      if (res.ok && data.success) {
        toast({
          ...
          description: data.message || "Your verification request has been submitted. An admin will review it shortly.",
        });
        fetchSettings();
      } else {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      // Validate account details based on payout method
      if (settingsForm.payoutMethod === "bank") {
        if (!settingsForm.accountDetails.accountNumber || !settingsForm.accountDetails.sortCode) {
          toast({
            title: "Validation Error",
            description: "Please fill in both account number and sort code for bank transfer",
            variant: "destructive",
          });
          setSubmitting(false);
          return;
        }
      } else if (settingsForm.payoutMethod === "paypal") {
        if (!settingsForm.accountDetails.email) {
          toast({
            title: "Validation Error",
      ...
      // Stripe doesn't need validation - handled via Stripe Connect
      // For Stripe, send empty account details
      const payload = {
        payoutMethod: settingsForm.payoutMethod,
        accountDetails: settingsForm.payoutMethod === "stripe" ? {} : settingsForm.accountDetails,
      };
  ...
  const handleRequestPayout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/business/payouts/request`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
  ...
  const handleConnectStripe = async () => {
    ...
    try {
      ...
      const linkRes = await fetch(`${API_BASE_URL}/business/stripe/onboarding-link`, {
        credentials: "include",
      });
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={isRequestDialogOpen} onOpenChange={setIsRequestDialogOpen}>
            <DialogTrigger asChild>
              <Button disabled={!settings || !settings.isVerified || earnings?.availableBalance === 0}>
          ...
          <CardContent>
            {stripeStatus ? (
              <div className="space-y-4">
                ...
                {settings && (
                  <Alert className={settings.isVerified ? "border-green-500 bg-green-50 dark:bg-green-950" : "border-amber-500 bg-amber-50 dark:bg-amber-950"}>
                    {settings.isVerified ? (
                      <>
                        <CheckCircle2 className="h-4 w-4 text-green-600" />
                        <AlertDescription className="text-green-800 dark:text-green-200">
                          Your payout account is verified. You can request payouts.
                        </AlertDescription>
                      </>
                    ) : (
                      <>
                        <AlertCircle className="h-4 w-4 text-amber-600" />
                        <AlertDescription className="text-amber-800 dark:text-amber-200">
                          Your payout account is not verified. Please request verification to enable payouts.
                        </AlertDescription>
                        <Button
                          variant="outline"
                          size="sm"
